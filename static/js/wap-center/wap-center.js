// JavaScript Document
window.wapCenterFed = window.wapCenterFed || {};
wapCenterFed.apiUrl= '';
wapCenterFed.payUrl= 'http://pay.m.b5m.com';
//订单列表页
wapCenterFed.orderList = function() {
	//获取订单列表
	this.init.getOrderList();
	//防止touchstart多次被触发
	this.repeatTouch();
};
//订单详情页
wapCenterFed.orderDetail = function() {
	//获取订单详情
	this.init.getOrderDetail();
	//订单详情按钮绑定
	this.init.orderDetailClick();
	//防止touchstart多次被触发
	this.repeatTouch();
};
//地址列表
wapCenterFed.addressList = function() {
	//获取地址列表
	this.init.getAddressList();
	//防止touchstart多次被触发
	this.repeatTouch();
};
//新增，修改地址
wapCenterFed.addressDetail = function() {
	//新增，修改地址
	this.init.setAddressDetail();
	//防止touchstart多次被触发
	this.repeatTouch();
};
//收藏列表
wapCenterFed.favoriteList = function() {
	//新增，修改地址
...
//配送地址表单验证
wapCenterFed.formVerification = function(){
	//必填
	...
	//显示错误提示
	var showErrorMsg = function(name,text){
		var html = '<em class="icon_warning"></em>'+ text;
		$("[name='"+name+"']").addClass('error');
		if(!$("#errorMsgBox").length){
			$("body").append( '<div id="errorMsgBox" class="error-msg-box"></div>');
		}
        var $errorMsgBox = $("#errorMsgBox");
        $errorMsgBox.html(html);
        console.log($errorMsgBox.width());
        var marginLeft =($errorMsgBox.width()/2*(-1))+"px";
        $errorMsgBox.css({ 'left':'50%','margin-left': marginLeft,'visibility':"visible"});
		setTimeout(function(){
	...
	//隐藏错误提示
	var hideErrorMsg = function(name){
		$("[name='"+name+"']").removeClass('error');
	};
	//所在地区
	if(required("provinceName")){
		hideErrorMsg("provinceName");
	}else{
		showErrorMsg("provinceName","所在省份不能为空！");
		return false;
	}
	if(required("cityName")){
		hideErrorMsg("cityName");
	}else{
		showErrorMsg("cityName","所在城市不能为空！");
		return false;
	}
	if(required("districtName")){
		hideErrorMsg("districtName");
	}else{
		showErrorMsg("districtName","所在地区不能为空！");
		return false;
	}
	//街道地址
	if(regularExp("detailAddress",/(^[0-9a-zA-Z\u4e00-\u9fa5]{5,120}$)/)){
		hideErrorMsg("detailAddress");
	}else{
		showErrorMsg("detailAddress","街道地址必须大于5个字符，小于120个字符！");
		return false;
	}
	//邮政编码
	if(regularExp("zipcode",/(^\s*$)|(^[1-9]\d{5}$)/)){
		hideErrorMsg("zipcode");
	}else{
		showErrorMsg("zipcode","请填写正确的邮编，必须6位！");
		return false;
	}
	//收货人姓名
	if(regularExp("userName",/^[a-zA-Z\u4e00-\u9fa5]{2,25}$/)){
		hideErrorMsg("userName");
	}else{
		showErrorMsg("userName","请输英文或中文姓名，必须大于2个字符，小于25个字符！");
		return false;
	}
	//手机号码
	if(regularExp("mobile",/(^1[3|4|5|7|8][0-9]\d{8}$)/)){
		hideErrorMsg("mobile");
	}else{
		showErrorMsg("mobile","请输入正确的11位手机号码！");
		return false;
	}
	//提交
	return true;
...
//防止touchstart多次被触发
wapCenterFed.repeatTouch = function(){
...
//ajax加载时的等待框
wapCenterFed.ajaxLoad = function(src){
...
//ajax加载公共方法
wapCenterFed.ajaxSubmit = function(data,successFun,errorFun){
	var sObj = {
			url      : "",
			type     : "GET",
			json     : "",
	...
	if ($.isFunction(successFun)){
		...
		$.ajaxJSONP({
			data    : sObj.json,
			url     : sObj.url+'?'+sObj.jsonp+'=?',
			success : successFun,
            error    :function(xhr, errorType, error){
                if($.isFunction(errorFun)){
                    errorFun();
                }else{
                    wapCenterFed.ajaxLoad('close');
                }
            }
		});
		return true;
...
/* 获取端口路径和提交方式 */
wapCenterFed.getResPort = function(name,key){
	var host =(name=="addressList" ? "" : wapCenterFed.apiUrl);
	var resObj = {
		addressList : {type:"GET",  url:"/address/find.html"},
		address     : {type:"GET",  url:"/dh/bang5mai/order/logistics/find.htm"},
		orderList   : {type:"GET",  url:"/dh/bang5mai/order/find.htm"},
		orderDetail : {type:"GET",  url:"/dh/bang5mai/order/findOrder.htm"},
		setStatus   : {type:"GET",  url:"/dh/bang5mai/order/goods/status.htm"},
        favoritesList : {type:"GET",  url:"/gc/user/favorites/data/query.htm"},
        deleFavorites : {type:"GET",  url:"/gc/user/favorites/data/delete.htm"}
	};
	return resObj.hasOwnProperty(name) ? (key=="url" ?  host+resObj[name][key] : resObj[name][key]): '';
};
//格式化日期格式YYYY-mm-dd HH:mm:ss
wapCenterFed.setDateTime = function(time){
	if(time && !isNaN(time)){
...
//省市，城市，区县 下拉框
wapCenterFed.setSelect = function(){
	var SPT = '请选择省市';
	var SCT = '请选择城市';
	var SAT = '请选择区/县';
	var ShowT = 1;
	...
	for (var i = 0; i < PCAN.length; i++) {
		...
		for (j = 0; j < TArea.length; j++) {
			...
			if (PCAA[i][j].length == 1) PCAA[i][j][1] = "直辖区";
	...
	if(arguments.length == 3){
		new PCAS(arguments[0],arguments[1],arguments[2]);
	}else{
		new PCAS(arguments[0],arguments[1],arguments[2],arguments[3],arguments[4],arguments[5]);
	}
};

//载入方法
wapCenterFed.init = (function(){
	return {
		//新增，修改地址
        setAddressDetail : function(){
			...
			var isDele = true;
            if(def=="true"){
                $(".deit-img").addClass("img-red");
            }
            //设置默认地址点击事件
			$(".order-deta-list").on("touchstart","#address_def",function(){
			    var $checkbox = $(this).children(".deit-img");
                if($checkbox.hasClass("img-red")){
                    $checkbox.removeClass("img-red");
                    $("#def").val('false');
                }else{
                    $checkbox.addClass("img-red");
                    $("#def").val('true');
                }
			});
            //保存按钮点击事件
            $(".order-deta-list").on("touchstart",".deit-save",function(){
                if(wapCenterFed.formVerification() && isSubmit){
					isSubmit = false;
					$("#saveForm").submit();
				}
            });
            //删除按钮点击事件
            $(".order-deta-list").on("touchstart",".deit-dele",function(){
				if(isDele){
					isDele = false;
					$("#deleForm").submit();
				}
		...
		getAddressList : function(){
			//生成地址列表方法
			var writeList = function(id,data){
				var html = "",
					def = "";
				if((data).length){
					$.each(data,function(i,m){
						if(m.def){
							def = '<div class="address-items checked"><a href="/address/toUpdate.html?addressId='+m.id+'&userId='+userId+'&sendRedirect="><em class="icon-checked"></em><h3>'+m.userName+'<span>'+m.mobile+'</span></h3><p><font>[默认]</font>'+m.provinceName+'  '+m.cityName+'  '+m.districtName+'  '+m.detailAddress+'</p><em class="icon-right"></em></a></div>\r\n';
						}else{
							html += '<div class="address-items '+(m.def ? 'checked' : '')+'"><a href="/address/toUpdate.html?addressId='+m.id+'&userId='+userId+'&sendRedirect="><em class="icon-checked"></em><h3>'+m.userName+'<span>'+m.mobile+'</span></h3><p>'+(m.def ? '<font>[默认]</font>' : '')+m.provinceName+'  '+m.cityName+'  '+m.districtName+'  '+m.detailAddress+'</p><em class="icon-right"></em></a></div>\r\n';
						}
					});
					html = def + html;
				}
			...
			var userId = $("#userId").val(),
				obj = {
					json : {'userId' : userId},
					url  : wapCenterFed.getResPort("addressList","url")
				};	
			wapCenterFed.ajaxLoad('open');
			wapCenterFed.ajaxSubmit(obj,function(data){
				if(data && data.code==100){
					writeList("address-container",data.data);
				}
				wapCenterFed.ajaxLoad('close');
			});
		},
		...
		getOrderList : function(){
			...
			var writeList = function(id,data){
				...
				if((data["all"]).length){
					$.each(data["all"],function(i,m){
						//01:已付款 00：未付款
						html += '<a class="myroder-mid" href="/user/info/orderDetail.html?orderNum='+m.orderNum+'">\r\n'
							 + '<span class="myroder-cont">订单号：<label>'+m.orderNum+'</label></span>\r\n'
							 + '<span class="myroder-cont">下单时间：<label>'+wapCenterFed.setDateTime(m.addTime)+'</label></span\r\n>'
							 + '<span class="myroder-pic"></span>\r\n'
							 + '<p class="myroder-total cfx">\r\n'
							 + '<span class="myroder-total-l">状态：<dfn>'+(m.status == "01" ? "已付款" : "未付款")+'</dfn></span\r\n>'
							 + '<span class="myroder-total-r">总计：<label>¥<dfn>'+(m.goodsSumPrice).toFixed(2)+'</dfn></label></span>\r\n'
							 + '</p>\r\n'
							 + '</a>\r\n';
					});
					$("."+id).removeClass("center-no-roder").addClass("center-roder").html(html);	
				}else{
					html = '<span class="order-pic"></span>\r\n'
						+ '<span class="no-order">您还没有订单哦！</span>\r\n'
						+ '<span class="low-price">各种低价<dfn>免邮</dfn>在向你招手</span>\r\n'
						+ '<a class="roder-button" href="http://m.b5m.com/">去逛逛</a>\r\n';
					$("."+id).removeClass("center-roder").addClass("center-no-roder").html(html);	
				}
			...
			var userId = $("#userId").val(),
				obj = {
					json : {'userId' : userId},
					url  : wapCenterFed.getResPort("orderList","url")
				};	
			wapCenterFed.ajaxLoad('open');
			wapCenterFed.ajaxSubmit(obj,function(data){
				if(data && data.code==100){
					writeList("center-list",data.data);
				}
				wapCenterFed.ajaxLoad('close');
			});
		},
		...
		getOrderDetail : function(){
			...
			//根据订单状态显示按钮
			var showBtn =  function(key ,orderNum){
				var html = "";
				switch (key){
					case "01":
						html = '<a class="btn-grade" data-ordernum="'+orderNum+'" data-status="11">取消订单</a>';
						break;
					case "03":
						html = '<a class="btn-grade btn-orange" data-ordernum="'+orderNum+'" data-status="04">确认收货</a>';
						break;
					default:	
					 	html = '';
						break;
			...
			//输出订单详情
			var writeDetail = function(id,data){
				var html = "<ul>\r\n";
				if(data.goodsList!= null && (data.goodsList).length){
					$.each(data.goodsList,function(i,m){
						html += '<li><a class="deta-list-img"><img src="'+m.goodsImgUrl+'" width="54" height="54" /></a>'
							 +  '<dl class="deta-list-mid">'
							 +  '<a>'+((m.goodsName).length > 30 ? (m.goodsName).substr(0,30)+'..' :  m.goodsName )+'</a>'
							 +  '<span>子订单号：<b>'+m.orderNum+'</b></span>'
							 +  '<span>'+m.goodsSpec+'</span>'
							 +  '<span>数量：<b>'+m.goodsNum+'</b></span>'
							 +  '<span>总计：<font>¥'+(m.goodsPrice).toFixed(2)+'</font></span>'
							 /*未支付 = "00"; 已付款 = "01";（取消订单） 已代购 = "02"; 已发货 = "03"（确认收货）; 已收货 = "04"; 无法退货 = "05"; 已取消 = "11"; 申请退货 = "12"; 已退款 = "13"; */
							 +  '<span>状态：<b>'+setStatus(m.status)+'</b></span>'
							 +  showBtn(m.status,m.orderNum)
							 +  '</dl></li>\r\n';
					});
				}
				html += "</ul>\r\n";
				//html += '<p>下单时间：<span>'+wapCenterFed.setDateTime(data.addTime)+'</span></p>\r\n';
				html += '<p>运费：<span>'+(data.fee==0 ? '免运费' : (data.fee).toFixed(2))+'</span></p>\r\n';
				html += '<p class="last-p">总计：<span><b>¥'+(data.goodsSumPrice).toFixed(2)+'</b></span></p>';
				$("."+id).html(html);	
				var obj = {
					json : {'logisticsId' : data.logisticsId},
					url  : wapCenterFed.getResPort("address","url")
				};	
				wapCenterFed.ajaxSubmit(obj,function(data_addre){
					var address = "";
					if(data_addre && data_addre.code==100){
						address += '<p>收件人：<span>'+data_addre.data.receiveUserName+'</span></p>\r\n';
						address += '<p>收货地址：<span>'+data_addre.data.receiveUserAddress+'</span></p>\r\n';
						$("."+id).children("ul").after(address);
					}
				});
			};
			//
			var showPayUrl = function(obj){
				var str = "";
				for(var key in obj){
					str += key + "=" + obj[key] + "&";
				}
				return wapCenterFed.payUrl + "/third/orderwap.htm?" + str;
			};
			var orderNum = $("#orderNum").val(),
				obj = {
					json : {'orderNum' : orderNum},
					url  : wapCenterFed.getResPort("orderDetail","url")
				};
			wapCenterFed.ajaxLoad('open');
			wapCenterFed.ajaxSubmit(obj,function(data){
				if(data && data.code==100){
					var objSearch = {
						...
						amount  : data.data.goodsSumPrice,
						subject : "zhifu" + data.data.orderNum,
						source  : 223,
						sign    : data.data.signature
					};
					var html = '<span>订单号：<b>'+data.data.orderNum+'</b></span>'
							 + '<span>下单时间：<b>'+wapCenterFed.setDateTime(data.data.addTime)+'</b></span>'
							 + '<span>状态：<dfn>'+(data.data.status == "01" ? "已付款" : "未付款")+'</dfn></span>'
							 + (data.data.status == '00' ? '<a href="'+showPayUrl(objSearch)+'" class="btn-orange" >去付款</a>' :'');
					//01:已付款 00：未付款
					$(".order-deta-titl").html(html);
					writeDetail('order-deta-list',data.data);
				}
				wapCenterFed.ajaxLoad('close');
			});
		},
		...
		orderDetailClick : function(){
			//订单状态设置按钮绑定
			$(".order-deta-list").on("touchstart",".btn-grade",function(e){
				var orderNum = $(this).data("ordernum"),
					...
					obj = {
						json : {'orderNum' : orderNum , 'status' : status},
						url  : wapCenterFed.getResPort("setStatus","url")
					};
				//显示对话框
				b5mwap.ui.msgDialog({
					content   : text[status], /*  内容  */
					isOpen    : true,/*  是否打开  */
					btn       : {
						'确定': function(){
							this.close();
							wapCenterFed.ajaxLoad('open');
							wapCenterFed.ajaxSubmit(obj,function(data){
								if(data.ok && data.code==100 && data.data=='SUCCESS'){
									wapCenterFed.init.getOrderDetail();
								}
							});
						},
						'取消': function(){
        ...
        getFavoriteList : function(){
            var page = 1,
                ...
                //userid = "1960e05a14fe4f0f8d8fce3f3e8fdd9f"
            //获取收藏列表
            var showListAjax = function(_this){
                if(!userid){
					 $(".center-no-roder").show();
					 wapCenterFed.ajaxLoad('close');
					 return false;
				}
               //debugger;
                var html = "",
                    obj = {
                        json : {"userId" : userid, "pageNum" : page,"pageSize":8},
                        url  : wapCenterFed.getResPort("favoritesList","url")
                    };
                wapCenterFed.ajaxSubmit(obj,function(data){
                    if(data.ok && (data.data.list).length && data.data.page.total){
                        $.each(data.data.list,function(i,m){
                            html += ' <li id="item' + m.ugcId + '">'
                                +  ' <a class="sale-list-pic" href="' + m.goodsUrl + '"><img src="' + m.picUrl + '"></a>'
                                +  ' <a class="sale-list-font" href="' + m.goodsUrl + '">' + ((m.title).length > 20 ? (m.title).substr(0,20)+'..' :  m.title ) + '</a>'
                                +  ' <p class="sale-list-price">收藏价：&yen;' + (m.collectionPrice).toFixed(2) + '</p>'
                                +  ' <p class="sale-list-price">现价：<span class="list-price-red">&yen;' + (m.price).toFixed(2) + '</span></p>'
                                +  ' <a class="right-btn" data-ugcid="' + m.ugcId + '"><span class="btn-dele">删除</span></a>'
                                +  ' </li>\n\r';
                        });
                        if(page==1){
                            $(".sale-list").show();
                        ...
                        page++;
                    }else if(data.ok && (data.data.list).length == 0 && data.data.page.total > 0){
						wapCenterFed.ajaxLoad('close');
					}else{
                        $(".center-no-roder").show();
                        wapCenterFed.ajaxLoad('close');
                    ...
                    if(_this){
                        ...
                        $(".sale-loading").children(".ui-refresh-icon").removeClass("ui-refresh-icon");
                    }
                });
            };
            //下拉加载数据
            ...
            //删除按钮绑定事件
            $(".data-list").on("touchstart",".right-btn",function(e){
                var _this = $(this),
                    ugcid = _this.data("ugcid"),
                    obj = {
                        json : {"userId" : userid, "ugcId" : ugcid},
                        url : wapCenterFed.getResPort("deleFavorites","url")
                    };
                //显示对话框
                b5mwap.ui.msgDialog({
                    content   : "确认是否删除收藏宝贝！", /*  内容  */
                    isOpen    : true,/*  是否打开  */
                    btn       : {
                        '确定': function(){
                            this.close();
