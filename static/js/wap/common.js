...
$(function() {
	...
	b5mwap.ui.loginBox = (function(){
		...
		//筛选页不显示
        if(/searchFilter/.test(location.pathname)){
           return false;
        }
	...
	//返回顶部，事件绑定
	$("body").on("touchstart",'#goTop',function(){
		window.scrollTo(0, 1);
        return false;
	});
	...
	//头部更多显示
	$(".h-right").on("touchstart",'#topMenu',function(){
		var $menu = $(".r-menu"),
			isShow = $menu.css("display"),
			headTop = $(".wap-header").offset().top,
			cssTop = headTop > 0 ? headTop+50 : 50;
	...
	b5mwap.ui.footBox = (function(){
        ...
        //用户中心\韩国馆\购物车，不需要加载
        if(location.host === 'cart.m.b5m.com' || location.host === 'ucenter.m.b5m.com'  || location.host === 'korea.b5m.com' /*|| /goodsDetail/.test(location.pathname)*/){
           return false;
        }
		var _html = "";
			
		_html = '<header id="footbar">'
			+ '<em class="footbar-logo"></em>'
			+ '<p class="footbar-text">点一点，安装手机版</p>'
			+ '<a href="http://a.b5m.com/downloadapp/b5mwap" class="footbar-download">立刻下载</a>'
	...
	//底部关闭绑定事件
	$("#footbar").on("touchstart",'.footbar-close',function(e){
		$('#footbar').remove();
		e.stopPropagation();
		return false;
	});
